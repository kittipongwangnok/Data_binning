...
InputFile = open("dpleo_20140112_run022kg5.inp",'r').readlines()
...
#Please change the input file
for line in open("dpleo_20140112_run022kg5.inp"):
    li=line.strip()
...
plt.ylabel('Relative flux')
plt.legend()
plt.savefig("Binned_Data_binning_dpleo_20140112_run022kg5.png")
plt.show()
...
for i in range(len(x_bin)):
    if x_bin[i] >= -0.04 and x_bin[i] <= 0.04 :
        BJD_time = (Phase*P) + t0
#    print('%0.6f %0.6f %0.6f %0.6f %0.0f %0.0f' %(BJD_time[i], exp_time[i], y_bin[i], dy_bin[i], weight_factor[i], subdivision[i]))
        New_lroche_input.append('%0.6f %0.6f %0.6f %0.6f %0.0f %0.0f' %(BJD_time[i], exp_time[i], y_bin[i], dy_bin[i], weight_factor[i], subdivision[i]))

data_lroche = New_lroche_input
f = open("Data_binning_dpleo_20140112_run022kg5.dat", 'w')
for i in range(len(data_lroche)):
    f.write(str(data_lroche[i])+ '\n')
f.close()
